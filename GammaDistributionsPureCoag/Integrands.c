...
double Inner_integrand(double xi, void *params){ 
	...
	integral1=pow(xi,1.0-homo_deg)*pow(dielta*(1+xi),homo_deg-3.0)*
				(gsl_sf_gamma_inc(3.0-homo_deg,dielta*zeta)
					-gsl_sf_gamma_inc(3.0-homo_deg,dielta*zeta*(1.0+xi)/xi)
				);
	if(xi<sqrt(ABSERR)){
		integral2=pow(xi,1.0-homo_deg)*pow(zeta,3.0-homo_deg)
			*exp(-dielta*zeta*(1+xi))/(1+xi);
	}
	else{
		integral2=pow(xi,-homo_deg)*pow(dielta*(1+xi),homo_deg-3.0)*
				(gsl_sf_gamma_inc(3.0-homo_deg,dielta*zeta)
					-gsl_sf_gamma_inc(3.0-homo_deg,dielta*zeta*(1.0+xi))
				);
	};
