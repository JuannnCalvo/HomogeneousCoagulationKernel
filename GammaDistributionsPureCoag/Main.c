...
//The profile to invert reads g(z)=z^-lambda e^(-delta z)
...
fscanf(CONSTANTS,"%lf %lf",&homogeneity_degree,&delta);
...
struct Xintegrand_params pars_Xintegrand={0,//bspline index
										...
										delta,//parameter at the exponent
...
for(i=0;i<NCOLLOCATIONPOINTS;i++){ 
	gsl_vector_set(data_vector,i,nu*pow(collocation_points[i],2.0-homogeneity_degree)
				*exp(-delta*collocation_points[i])); 
};
