...
double homogeneity_degree=0.0; //test case with a constant kernel
...
fscanf(PARS_FILE,"%lf",&homogeneity_degree);

//Abort when the homogeneity degree is not suitable
//Handle the homogeneity one case separately
...
//We use GSL vectors to be able to invoke 
...
///////////////
//B-splines II: regularizing matrix and kernel expansion
...
for(i=0;i<NCOLLOCATIONPOINTS;i++){ 
    gsl_vector_set(data_vector,i,
                    nu*collocation_points[i]*collocation_points[i]
                        *linear_Bcombination(collocation_points[i],&params_fit)
                   );
