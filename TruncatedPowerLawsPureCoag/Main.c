...
//The profile to invert reads g(z)= z^(-1-lambda) chi_{(0,z_{max})}(z)

double homogeneity_degree=-0.5; //aka lambda above
...
double nu=-1.0/(1.0-homogeneity_degree);
...
X_integrands.function=&Inner_integrand;
...
for(i=0;i<NCOLLOCATIONPOINTS;i++){ 
	gsl_vector_set(data_vector,i,nu*cut(collocation_points[i])
				*pow(collocation_points[i],1.0-homogeneity_degree)); 
};
...
gsl_multifit_linear_solve(lambda_l, //regularization weigth is lambda^2 
			coeff_matrix, 
			data_vector, 
...
struct bspline_comb_params params_linearcomb_lcurve 
			={kernel_expansion_coefs_lcurve,bw,B,NCOEFFS};
...
gsl_multifit_linear_solve(lambda_gcv, //regularization weigth is lambda^2 
			coeff_matrix,
			data_vector,
...
struct bspline_comb_params params_linearcomb_gcv ={kernel_expansion_coefs_gcv,bw,B,NCOEFFS};
//{coeff list, b-workspace, B-vector, number of functions to combine}
