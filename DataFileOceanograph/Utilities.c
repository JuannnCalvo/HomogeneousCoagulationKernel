...
double integrand(double y, void * params) { 
  ...
  struct bspline_comb_params *pars_datafit;
  double landa, xi, f;

  p= (struct inner_integral_params *) params;
  landa = (p->homogeneity_degree); 
  xi = (p->xi);
  pars_datafit=(p->params_fit);
  
  f =pow(y,2+landa)*linear_Bcombination (y,pars_datafit)  
        *linear_Bcombination (y*xi,pars_datafit);
...
double StokesIntegrand(double y, void * params) { 

  double f;
  struct bspline_comb_params *params_fit;

  params_fit= (struct bspline_comb_params *) params;

  f =pow(y,5.0/3.0)*linear_Bcombination (y,params_fit);  

  return f;
};


//Form integrands for the coefficient matrix of the discretized inverse problem
double Xintegrand (double xi, void *params){ 
  ...
  landa=(p->homogeneity_degree);
  x=(p->collocation_point);
 
  pars_datafit=(p->pars_data);


  //Create the functions to integrate
  struct inner_integral_params pars_int ={landa,xi,pars_datafit};
  ...
  Int.params = &pars_int;
   ...
   if(xi>sqrt(ABSERR)){ 
      ...
      integral2=xi*pow(x,3+landa)*linear_Bcombination(x,pars_datafit)
            *linear_Bcombination (xi*x,pars_datafit)/(1+xi);
    };
