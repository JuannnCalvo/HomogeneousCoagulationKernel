...
/////////////////
size_t i, j;
FILE *CONSTANTS; //input of method-parameter values
double lambda =1; //The regularization weight is lambda^2

//The profile to invert reads g(z)=z^(-3/2) e^(-z/2)/sqrt(2*PI)
...
////////////////
//Concerning the multifit routine
...
for(i=0;i<NCOLLOCATIONPOINTS;i++){ 
	gsl_vector_set(data_vector,i,
					-sqrt(2.0/PI)*sqrt(collocation_points[i])*exp(-0.5*collocation_points[i])
					); 
};
...
///////////////
// Solve using the given value lambda. First solve the problem in standard form:
gsl_multifit_linear_solve(lambda, //regularization weight is lambda^2
			coeff_matrix, 
			data_vector, 
...
for (i = 0; i < 1+NCOLLOCATIONPOINTS; ++i){
	printf("%f %f\n", (1.0/NCOLLOCATIONPOINTS)*i,
		linear_Bcombination ((1.0/NCOLLOCATIONPOINTS)*i,&params_linearcomb)
		);
};
